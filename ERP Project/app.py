...
if menu == "Dashboard":
    ...
    with col_title:
        st.markdown("<h1 style='padding-top: 20px;'>Accounting Financial Dashboard</h1>", unsafe_allow_html=True)
    ...
    if accounts:
        ...
        df_accounts['type'] = df_accounts['type_id'].map(type_map)
        
        col1, col2, col3 = st.columns(3)
        total_assets = df_accounts[df_accounts['type'] == 'Asset']['balance'].sum()
        total_liabilities = df_accounts[df_accounts['type'] == 'Liability']['balance'].sum()
        total_equity = df_accounts[df_accounts['type'] == 'Equity']['balance'].sum()
        ...
        st.dataframe(df_accounts[['account_id', 'account_name', 'type', 'balance']], use_container_width=True)
        
        # Download PDF Button
        st.divider()
        st.subheader("Reports")
        pdf_output = generate_trial_balance_pdf(df_accounts)
...
# --- Chart of Accounts ---
    ...
    with st.expander("⚙️ Manage Account Types"):
        ...
        if types:
            with st.form("delete_type"):
                type_to_del = st.selectbox("Select Type to Delete", options=[t.type_id for t in types], format_func=lambda x: next(t.category_name for t in types if t.type_id == x))
    ...
    if accounts:
        ...
        st.dataframe(df[['account_id', 'account_name', 'account_code', 'type', 'balance']], use_container_width=True)
    ...
    with col1:
        with st.expander("➕ Add New Account"):
            with st.form("add_account"):
                ...
                acc_type = st.selectbox("Account Type", options=list(type_options.keys()), format_func=lambda x: type_options[x])
                if st.form_submit_button("Create Account"):
                    with Session(engine) as session:
                        session.add(Account(account_id=acc_id, account_name=acc_name, account_code=acc_code, type_id=acc_type))
                        session.commit()
                        st.rerun()
    with col2:
        if accounts:
            with st.expander("🗑️ Delete Account"):
                with st.form("delete_account"):
                    acc_to_del = st.selectbox("Select Account to Delete", options=[a.account_id for a in accounts], format_func=lambda x: f"{x} - {next(a.account_name for a in accounts if a.account_id == x)}")
        ...
        if employees:
            with st.expander("🗑️ Delete Employee"):
                with st.form("delete_employee"):
                    emp_to_del = st.selectbox("Select Employee to Delete", options=[e.employee_id for e in employees], format_func=lambda x: next(e.name for e in employees if e.employee_id == x))
        ...
        if customers:
            with st.expander("🗑️ Delete Customer"):
                with st.form("delete_customer"):
                    cust_to_del = st.selectbox("Select Customer to Delete", options=[c.customer_id for c in customers], format_func=lambda x: next(c.name for c in customers if c.customer_id == x))
        ...
        if vendors:
            with st.expander("🗑️ Delete Vendor"):
                with st.form("delete_vendor"):
                    ven_to_del = st.selectbox("Select Vendor to Delete", options=[v.vendor_id for v in vendors], format_func=lambda x: next(v.company_name for v in vendors if v.vendor_id == x))
        ...
        if products:
            with st.expander("🗑️ Delete Product"):
                with st.form("delete_product"):
                    prod_to_del = st.selectbox("Select Product to Delete", options=[p.product_id for p in products], format_func=lambda x: next(p.name for p in products if p.product_id == x))
        ...
        if taxes:
            with st.expander("🗑️ Delete Tax"):
                with st.form("delete_tax"):
                    tax_to_del = st.selectbox("Select Tax to Delete", options=[t.tax_id for t in taxes], format_func=lambda x: next(t.tax_name for t in taxes if t.tax_id == x))
        ...
        if methods:
            with st.expander("🗑️ Delete Payment Method"):
                with st.form("delete_pm"):
                    pm_to_del = st.selectbox("Select Payment Method to Delete", options=[m.payment_id for m in methods], format_func=lambda x: next(m.method_name for m in methods if m.payment_id == x))
...
# --- Invoices ---
    ...
    if invoices:
        ...
        st.dataframe(df[['invoice_id', 'date', 'total', 'status', 'customer', 'employee']], use_container_width=True)
    ...
    with col1:
        with st.expander("➕ Create New Invoice"):
            with st.form("add_invoice"):
                ...
                cust_id = st.selectbox("Customer", options=list(cust_map.keys()), format_func=lambda x: cust_map[x])
                emp_id = st.selectbox("Assigned Employee", options=list(emp_map.keys()), format_func=lambda x: emp_map[x])
                if st.form_submit_button("Create Invoice"):
                    with Session(engine) as session:
                        session.add(Invoice(date=inv_date, total=total, status=status, customer_id=cust_id, employee_id=emp_id))
                        session.commit()
                        st.rerun()
    with col2:
        if invoices:
            with st.expander("🗑️ Delete Invoice"):
                with st.form("delete_invoice"):
                    inv_to_del = st.selectbox("Select Invoice to Delete", options=[i.invoice_id for i in invoices], format_func=lambda x: f"Inv #{x} - {cust_map.get(next(inv.customer_id for inv in invoices if inv.invoice_id == x), 'Unknown')}")
    ...
    if not accounts:
        ...
        with st.form("journal_entry_form"):
            ...
            for i in range(num_lines):
                ...
                acc_id = row_c1.selectbox(f"Account {i+1}", options=list(account_options.keys()), format_func=lambda x: account_options[x], key=f"acc_{i}")
                deb = row_c2.number_input(f"Debit {i+1}", min_value=0.0, format="%.2f", key=f"deb_{i}")
                cre = row_c3.number_input(f"Credit {i+1}", min_value=0.0, format="%.2f", key=f"cre_{i}")
                p_id = row_c4.selectbox(f"Product {i+1}", options=list(prod_map.keys()), format_func=lambda x: prod_map[x], key=f"prod_{i}")
                t_id = row_c5.selectbox(f"Tax {i+1}", options=list(tax_map.keys()), format_func=lambda x: tax_map[x], key=f"tax_{i}")
        ...
        # Add Delete for Journal Entry
        entries = get_all(JournalEntry)
        if entries:
            with st.expander("🗑️ Delete Journal Entry"):
                with st.form("delete_entry"):
                    entry_to_del = st.selectbox("Select Entry to Delete", options=[e.entry_id for e in entries], format_func=lambda x: f"ID {x} - {next(e.description for e in entries if e.entry_id == x)}")
                    if st.form_submit_button("�️ Delete Journal Entry"):
                        # First delete journal items
                        with Session(engine) as session:
                            items = session.exec(select(JournalItem).where(JournalItem.entry_id == entry_to_del)).all()
                            for item in items:
                                session.delete(item)
                            session.commit()
...
# --- General Ledger ---
    ...
    with Session(engine) as session:
        statement = select(JournalItem, Account.account_name, JournalEntry.date, JournalEntry.description)\
